...
uniform Materials {
  ...
  vec4 roughnessMetalnessNormalScale[NUM_MATERIALS];
    ...
    vec4 diffuseNormalMapSize[${Math.max(
      defines.NUM_DIFFUSE_MAPS,
      defines.NUM_NORMAL_MAPS
    )}];
  #endif
...
`;
  ...
  function mergeMeshesToGeometry(meshes) {
    ...
    const { geometry, materialIndices } = mergeGeometry(
  ...
  function mergeGeometry(geometryAndMaterialIndex, vertexCount, indexCount) {
    const position = new THREE$1.BufferAttribute(
      new Float32Array(3 * vertexCount),
      3,
      false
    );
    const normal = new THREE$1.BufferAttribute(
      new Float32Array(3 * vertexCount),
      3,
      false
    );
    const uv = new THREE$1.BufferAttribute(
      new Float32Array(2 * vertexCount),
      2,
      false
    );
    const index = new THREE$1.BufferAttribute(
  ...
  function makeTexture(gl, params) {
    ...
    const isFloatArray = storage === "float" || data instanceof Float32Array;
  ...
  */
  ...
  function makeRayTracingShader({
    ...
    function setStrataCount(strataCount) {
      ...
      if (strataCount > 1 && strataCount !== samples.strataCount) {
        ...
        samples = makeStratifiedSamplerCombined(
          strataCount,
          samplingDimensions
        );
      } else {
    ...
    function useStratifiedSampling(stratifiedSampling) {
      gl.useProgram(program);
      gl.uniform1f(
        uniforms.useStratifiedSampling,
        stratifiedSampling ? 1.0 : 0.0
      );
    }
  ...
  function makeProgramFromScene({
    ...
    const { meshes, directionalLights, environmentLights } = decomposeScene(
      scene
    );
    if (meshes.length === 0) {
    ...
    const { geometry, materials, materialIndices } = mergeMeshesToGeometry(
      meshes
    );
    ...
    if (maps.map.textures.length > 0) {
      const { relativeSizes, texture } = makeTextureArray$1(
        gl,
        maps.map.textures,
        true
      );
    ...
    scene.traverse(child => {
      if (child.isMesh) {
        if (!child.geometry || !child.geometry.getAttribute("position")) {
          console.warn(
            child,
            "must have a geometry property with a position attribute"
          );
      ...
      if (child.isEnvironmentLight) {
        if (environmentLights.length > 1) {
          ...
          console.warn(
            child,
            "environment light does not use THREE.RGBEEncoding"
          );
        }
      }
    });
  ...
  function isHDRTexture(texture) {
    return (
      texture.map &&
      texture.map.image &&
      (texture.map.encoding === THREE$1.RGBEEncoding ||
        texture.map.encoding === THREE$1.LinearEncoding)
    );
  }
...
const vec3 uncharted2WhitePoint = 1.0 / uncharted2Helper(vec3(${
      defines.whitePoint
    }));
...
`;
  ...
  function makeRenderingPipeline({
    ...
    const rayTracingShader = makeRayTracingShader({
      gl,
      optionalExtensions,
      fullscreenQuad,
      textureAllocator,
    ...
    function setPreviewBufferDimensions() {
      ...
      const numPixelsForPreview =
        desiredTimeForPreview / tileRender.getTimePerPixel();
      const previewWidth = clamp(
        Math.sqrt(numPixelsForPreview * aspectRatio),
        1,
        hdrBuffer.width
      );
    ...
    function camerasEqual(cam1, cam2) {
      return (
        numberArraysEqual(
          cam1.matrixWorld.elements,
          cam2.matrixWorld.elements
        ) &&
